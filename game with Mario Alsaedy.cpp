...
void drawMario(float x, float y) {
    ...
    float skinColorR = 0.85f, skinColorG = 0.65f, skinColorB = 0.55f;
    float shirtColorR = 1.0f, shirtColorG = 1.0f, shirtColorB = 1.0f;
    float pantsColorR = 1.0f, pantsColorG = 1.0f, pantsColorB = 1.0f;
    float shoeColorR = 0.4f, shoeColorG = 0.2f, shoeColorB = 0.0f;
    float hatColorR = 1.0f, hatColorG = 1.0f, hatColorB = 1.0f;

    float bodyWidth = 25.0f;
    float bodyHeight = 60.0f;
    float headRadius = 15.0f;
    float armWidth = 5.0f, armHeight = bodyHeight / 1.5f;
    float legWidth = 12.5f, legHeight = bodyHeight / 2.0f;
    ...
    glColor3f(shirtColorR, shirtColorG, shirtColorB);
    drawRectangle(x, y, bodyWidth, bodyHeight);

    // Draw the head (circle)
    glColor3f(skinColorR, skinColorG, skinColorB);
    glBegin(GL_POLYGON);
    ...
    // Draw three points in a vertical line on the top part of the body
    glPointSize(2.0f);
    glColor3f(0.1f, 0.1f, 0.1f);
    glBegin(GL_POINTS);
    glVertex2f(x + bodyWidth / 2.0f, y-43 + bodyHeight + 5.0f);
    glVertex2f(x + bodyWidth / 2.0f, y-43 + bodyHeight + 15.0f);
    glVertex2f(x + bodyWidth / 2.0f, y-43 + bodyHeight + 25.0f);
    glEnd();
    ...
    drawRectangle(x - (armWidth / 2.0f), y-9 + bodyHeight / 3.0f, armWidth, armHeight);
    drawRectangle(x + bodyWidth - (armWidth / 2.0f), y-9 + bodyHeight / 3.0f, armWidth, armHeight);

    // Draw the legs
    glColor3f(pantsColorR, pantsColorG, pantsColorB);
    drawRectangle(x + (bodyWidth / 2.0f) - legWidth, y - legHeight, legWidth, legHeight);
    drawRectangle(x + (bodyWidth / 2.0f), y - legHeight, legWidth, legHeight);

    // Draw the shoes
    glColor3f(shoeColorR, shoeColorG, shoeColorB);
    drawRectangle(x-2 + (bodyWidth / 2.0f) - legWidth, y - legHeight - shoeHeight, shoeWidth, shoeHeight);
    drawRectangle(x+3 + (bodyWidth / 2.0f), y - legHeight - shoeHeight, shoeWidth, shoeHeight);

    // Draw the hat (semi-circle)
    glColor3f(hatColorR, hatColorG, hatColorB);
    float hatCenterX = x + 12;
    float hatCenterY = y + 5 + bodyHeight + hatHeight / 2.0f;
    float hatRadius = hatWidth / 2.0f;
    int numSegmentsHat = 100;
